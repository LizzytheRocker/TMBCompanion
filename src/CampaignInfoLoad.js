...
class CampaignInfoLoad extends React.Component {
	constructor() {
		super();
		this.state = { campaign: new Campaign }
	}
	...
	async submitCampaign() {
		...
		this.state.campaign.tyrants_defeated = document.getElementById('CampaignInfoTyrantsDefeated').value;
		this.state.campaign.tyrant = document.getElementById('CampaignInfoTyrant').value;
		try {
			this.state.campaign.starting_point = Number(document.getElementById('CampaignInfoStartingPoint').value);
		}
		catch (error) {
			if (error.name == "TypeError") {
				alert("Make sure to only input whole numbers for the starting point!");
				document.getElementById("Download").removeAttribute("href");
				document.getElementById("Download").removeAttribute("download");
				return;
			}
			if (error.name == "RangeError") {
				alert("Make sure the starting point is at least 0!");
				document.getElementById("Download").removeAttribute("href");
				document.getElementById("Download").removeAttribute("download");
				return;
			}
		...
		try {
			this.state.campaign.required_progress = Number(document.getElementById('CampaignInfoRequiredProgressPoints').value);
		}
		catch (error) {
			if (error.name == "TypeError") {
				alert("Make sure to only input whole numbers for the required progress points!");
				document.getElementById("Download").removeAttribute("href");
				document.getElementById("Download").removeAttribute("download");
				return;
			}
			if (error.name == "RangeError") {
				alert("Make sure the required progress points are at least 0!");
				document.getElementById("Download").removeAttribute("href");
				document.getElementById("Download").removeAttribute("download");
				return;
			}
		}
		// game handled below characters
		// char 1
		try {
			...
			this.state.campaign.addCharacter(char);
		}
		catch (error) {
			if (error.name == "TypeError") {
		...
		if (Boolean(document.getElementById('CharInfo2').checked)) {
			try {
				...
				this.state.campaign.addCharacter(char2);
			}
			catch (error) {
				if (error.name == "TypeError") {
		...
		if (Boolean(document.getElementById('CharInfo3').checked)) {
			try {
				...
				this.state.campaign.addCharacter(char3);
			}
			catch (error) {
				if (error.name == "TypeError") {
		...
		if (Boolean(document.getElementById('CharInfo4').checked)) {
			try {
				...
				this.state.campaign.addCharacter(char4);
			}
			catch (error) {
				if (error.name == "TypeError") {
		...
		if (Boolean(document.getElementById('CampaignInfoOngoingGame').checked)) {
			var gameTemp = new Game;
			gameTemp.save_name = document.getElementById('GameInfoName').value;
			gameTemp.tyrant = document.getElementById('GameInfoTyrant').value;
			try {
				gameTemp.game.day_count = Number(document.getElementById('GameInfoDayCount').value);
			}
			catch (error) {
				if (error.name == "TypeError") {
			...
			try {
				gameTemp.progress_points = Number(document.getElementById('GameInfoProgPoints').value);
			}
			catch (error) {
				if (error.name == "TypeError") {
			...
			gameTemp.encounters_completed = document.getElementById('GameInfoEncountersCompleted').value;
			gameTemp.encounters_remaining = document.getElementById('GameInfoEncountersRemaining').value;
			gameTemp.loot_used = document.getElementById('GameInfoLootUsed').value;
    ...
    for (var i = 0, f; f = files[i]; i++) {
			...
			try {
				...
				} else if (ftype != "character" && ftype != "game" && ftype != "campaign") {
			...
			<div className='CampaignInfo'>
				<form>
					<div class="row">
						...
						<div class="row">
							<div class="col">
								<h2>Adventure Effect</h2>
								<input
									id='CampaignInfoAdventureEffect'
									type='text'
								/>
							</div>
							<div class="col">
								<h2>Boons</h2>
								<input
									id='CampaignInfoBoons'
									type='text'
								/>
							</div>
							<div class="col">
								<h2>Tyrants Defeated</h2>
								<input
									id='CampaignInfoTyrantsDefeated'
									type='text'
								/>
							</div>
						</div>
						...
						<div class="row">
							<div class="col">
								<h2>Next/Current Campaign Tyrant</h2>
								<input
									id='CampaignInfoTyrant'
									type='text'
								/>
							</div>
							<div class="col">
								...
								<input
									id='CampaignInfoStartingPoint'
									type='number'
								/>
							</div>
							<div class="col">
								<h2>Required Progress Points</h2>
								<input
									id='CampaignInfoRequiredProgressPoints'
									type='number'
								/>
							</div>
						</div>
						...
						<div class="row">
							<div class="col">
								<h2>Is there an ongoing game?</h2>
								<input
									id='CampaignInfoOngoingGame'
									type='checkbox'
								/>
							</div>
							<div class="col">
...
export default CampaignInfoLoad